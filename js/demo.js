...
const DEMO = (() => {
  ...
  function build() {
    ...
    ev.push([1.6, 1.75, (s, u) => drop(s, w * 0.24, h * 0.27, w * 0.02, 0.03, 0, 0.5)]); // ultramarine
    ev.push([2.1, 2.25, (s, u) => drop(s, w * 0.33, h * 0.33, w * 0.02, 0.03, 5, 0.5)]); // rose
    ev.push([2.6, 2.7, (s, u) => drop(s, w * 0.28, h * 0.24, w * 0.015, 0.025, 3, 0.45)]); // yellow
    ...
    ev.push([8.2, 8.9, (s, u) => {
      ...
      drop(s, x, h * 0.22, w * 0.03, 0.05, 3, 0.4, 1.0); // hansa yellow
    }]);
    ev.push([9.2, 9.9, (s, u) => {
      const x = w * (0.65 + u * 0.2);
      drop(s, x, h * 0.28, w * 0.025, 0.03, 1, 0.4, 0.9); // cerulean -> mixes to green
    }]);
